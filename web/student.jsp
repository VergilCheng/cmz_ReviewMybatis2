...
<%--静态资源访问，全路径要加项目名--%>
<script type="text/javascript" src="/cmz_ReviewMybatis2/js/jquery-1.7.2.js"></script>
<script type="text/javascript">
    <%--必须保证写在jsp文件中，如果写在js文件中由于js文件服务器不会进行任何解析操作
        直接发送给客户端浏览器，所以如果写在js文件中认为${}是字符串}--%>
    $(function () {
        ...
        $("button").click(function(){
            location.href="showSandT?pageSize="+pageSize+"&pageNumber=1&tname="+$(":text[name='tname']").val()+"&sname="+$(":text[name='sname']").val();
        });
        //单选按钮点击事件
        ...
        $(".page_a:eq(0)").click(function(){
            ...
            if(pageNumber>=1){
                location.href="showSandT?pageSize="+pageSize+"&pageNumber="+pageNumber+"&tname="+$(":text[name='tname']").val()+"&sname="+$(":text[name='sname']").val();
            }else{
                pageNumber = 1;
            }
            return false;
